...
class StreamingTweets:
	...
	def get_stream(self, api, auth, home, away, users_ids, runtime):
		listen = SListener(
			api=api,
			team1=home,
			team2=away,
	...
	def start_streaming(self, home, away, users, runtime):
		try:
			api, auth = self.authorize_api(self.secrets)
			users_ids = self.get_user_ids(users, api)
			self.stream = self.get_stream(api, auth, home, away, users_ids, runtime)
	...
	def start(self, home, away, users = [], runtime=30):
		try:
			self.start_streaming(home, away, users, runtime)
